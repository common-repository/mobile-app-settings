...
if ($keyword == "") {
    $XML .= "<error>-2</error>";
    $XML .= "<errorMessage>No Search Keyword</errorMessage>";
} else if (!empty($alc_api_options['cat'])) {
    /* 	Query posts for the latest articles	 */
    $args = array('post_type' => 'post', 'category__in' => $alc_api_options['cat'], 'posts_per_page' => $posts_per_page, 'paged' => $page, s => $keyword);
