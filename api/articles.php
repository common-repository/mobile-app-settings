<?php
    ...
    $cat_id = (INT) strip_tags($_GET['cid']);
    $category = get_category($cat_id);

    $XML .= "<cid>".$cat_id."</cid>"; // just return selected category id
    ...
    if (!empty($category)) {
        ...
        //print_r($category);
        //echo '</pre>';
        if (in_array($cat_id, $alc_api_options['cat'])) {

            /* 	Query posts from category	 */
            $args = array('post_type' => 'post', 'cat' => $cat_id, 'posts_per_page' => $posts_per_page, 'paged' => $page);
            ...
            $XML .= "<error>-1</error>";
            $XML .= "<errorMessage>This category is prohibited for export</errorMessage>";
        }
    } else {

        $XML .= "<error>-1</error>";
        $XML .= "<errorMessage>Invaild category id</errorMessage>";
    }
    $XML .= "</articles>";
